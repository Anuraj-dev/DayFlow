...
- Employee and HR land on different dashboards. Employee, attendance, leave, and payroll pages keep both role states.
...
| Backend tests | `docker compose exec backend pytest` | Host Python may be 3.14; images pin 3.12. Run tests inside the container. |
| API only, UI on host | `docker compose up db backend` then `npm --prefix frontend run dev` | Vite proxies `/api` to `localhost:8000` unless `API_PROXY_TARGET` is set. |
...
2. **UI-only permission** — Employee opens `/employees/:id` or another person's payroll by editing the URL. Cause: route guard without a matching server check. Correct path: FastAPI dependency filters by membership and employee-id; UI hides the entry point as well.
