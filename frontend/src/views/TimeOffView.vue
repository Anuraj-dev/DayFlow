...
import { CalendarPlusIcon, ClipboardListIcon, BriefcaseBusinessIcon } from '@lucide/vue'
...
const cancelConfirmationId = ref('')
...
async function decide(id: string, decision: 'approve' | 'reject') {
  if (decidingRequestId.value) return
  actionError.value = ''
  actionStatus.value = ''
...
function requestCancel(id: string) {
  ...
  cancelConfirmationId.value = id
}

function keepRequest() {
  cancelConfirmationId.value = ''
}

async function cancelRequest(id: string) {
  if (cancellingRequestId.value || cancelConfirmationId.value !== id) return
  actionError.value = ''
  actionStatus.value = ''
  cancellingRequestId.value = id
  try {
  ...
  <section class="sheet">
    ...
    <div v-else class="grid gap-6">
      ...
      <div v-if="session.isHr">
        ...
        <template v-else>
          <div
            class="mb-2 flex flex-wrap items-center justify-between gap-2 text-sm text-[#495057]"
          >
          ...
          <Table>
            ...
            <TableBody>
              ...
              >
                ...
                <TableCell
                  >{{ formatDate(row.starts_on) }} to {{ formatDate(row.ends_on) }}</TableCell
                >
          ...
          >
            <h2 id="leave-review-title" class="mt-0">
              Review {{ selectedRequest.employee_name ?? 'employee request' }}
            </h2>
            <div class="grid gap-3 sm:grid-cols-2">
              <p class="m-0">
                <strong>Type</strong><br />{{ leaveTypeLabel(selectedRequest.leave_type) }}
              </p>
              <p class="m-0">
                <strong>Dates</strong><br />{{ formatDate(selectedRequest.starts_on) }} to
                {{ formatDate(selectedRequest.ends_on) }}
              </p>
              <p class="m-0">
                <strong>Counted days</strong><br />{{
                  selectedRequest.counted_days ?? 'Not calculated'
                }}
              </p>
              <p class="m-0">
                <strong>Overlap</strong><br />{{
                  requestConflicts(selectedRequest) ? 'Conflict found' : 'No overlap'
                }}
              </p>
            </div>
            <p v-if="selectedRequest.reason" class="mt-3 mb-0">
              <strong>Employee reason</strong><br />{{ selectedRequest.reason }}
            </p>
            ...
            <div class="mt-3 flex flex-wrap gap-2">
              <Button type="button" variant="outline" :disabled="Boolean(decidingRequestId)" @click="decide(selectedRequest.id, 'approve')"
      ...
      <template v-else>
        <div
          class="grid gap-6 border-b border-border pb-6 lg:grid-cols-[minmax(16rem,0.75fr)_minmax(28rem,1.25fr)] lg:gap-0"
        >
          <section
            class="lg:border-r lg:border-border lg:pr-8"
            aria-labelledby="leave-balances-title"
          >
            <div class="mb-3 flex items-center gap-2">
              <BriefcaseBusinessIcon class="size-5" :stroke-width="1.75" aria-hidden="true" />
              <h2 id="leave-balances-title" class="m-0">Leave balances</h2>
            </div>
            <div v-if="data?.balances.length" class="divide-y divide-border">
              <div
                v-for="row in data.balances"
                :key="row.leave_type"
            ...
            <p v-else class="m-0 text-muted-foreground">No balances yet.</p>
          </section>

          <form class="grid content-start gap-3 lg:pl-8" @submit.prevent="submitRequest">
            <div class="flex items-center gap-2">
              <CalendarPlusIcon class="size-5" :stroke-width="1.75" aria-hidden="true" />
              <h2 class="m-0">Submit a request</h2>
            </div>
            ...
            <div
              v-if="draftKind"
              class="flex flex-wrap items-center gap-x-4 gap-y-2 border-y border-border py-2 text-sm"
            >
              <StatusBadge :label="draftLabel" :tone="statusTone(draftLabel)" />
              <span
                >Counted days: <strong class="tabular-nums">{{ draftCounted }}</strong></span
              >
              <span v-if="draftKind === 'draft'" class="text-muted-foreground">
            ...
            <div class="flex flex-wrap items-center gap-3">
              ...
              <p v-if="!canSubmit && startsOn && endsOn" class="m-0 text-sm text-muted-foreground">
        ...
        <section aria-labelledby="leave-requests-title">
          <div class="mb-2 flex items-center gap-2">
            <ClipboardListIcon class="size-5" :stroke-width="1.75" aria-hidden="true" />
            <h2 id="leave-requests-title" class="m-0">Requests</h2>
          </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                <TableRow v-for="row in data?.requests ?? []" :key="row.id">
                  ...
                  <TableCell>
                    <div v-if="row.status.toUpperCase() === 'PENDING'" class="flex flex-wrap gap-2">
                    <Button
                      type="button"
                      size="sm"
                      variant="outline"
                      :disabled="Boolean(cancellingRequestId)"
                      @click="cancelConfirmationId === row.id ? cancelRequest(row.id) : requestCancel(row.id)"
                    >
                      {{ cancellingRequestId === row.id ? 'Cancelling…' : cancelConfirmationId === row.id ? 'Confirm cancel' : 'Cancel request' }}
                    </Button>
                    <Button v-if="cancelConfirmationId === row.id" type="button" size="sm" variant="ghost" :disabled="Boolean(cancellingRequestId)" @click="keepRequest">Keep request</Button>
                    </div>
                  </TableCell>
                </TableRow>
              </TableBody>
          ...
          <div v-if="data?.requests.length" class="mobile-record-list">
            <article v-for="row in data.requests" :key="row.id" class="mobile-record">
              ...
              <div v-if="row.status.toUpperCase() === 'PENDING'" class="grid gap-2">
              <Button
                type="button"
                variant="outline"
                :disabled="Boolean(cancellingRequestId)"
              ...
              <Button v-if="cancelConfirmationId === row.id" type="button" variant="ghost" :disabled="Boolean(cancellingRequestId)" @click="keepRequest">Keep request</Button>
              </div>
            </article>
          </div>
        </section>
