...
import { AlertTriangleIcon, CalendarDaysIcon, Clock3Icon } from '@lucide/vue'
...
const currentSession = computed<AttendanceSession | null>(
  () => data.value?.sessions.find((row) => row.work_date === localDateKey()) ?? null,
)

const todayLabel = computed(() => {
  if (!currentSession.value) return attendanceStatusLabel('not_checked_in')
  if (data.value?.open_session?.id === currentSession.value.id) return 'Checked in'
  if (currentSession.value.check_out_at) return 'Checked out'
  return sessionLabel(currentSession.value)
})
...
const weekSummary = computed(() => {
  ...
  return {
    ...
    complete: sessions.filter((row) => Boolean(row.check_out_at)).length,
    attention: sessions.filter((row) => sessionLabel(row) === 'Missing check-out').length,
  }
})
  ...
  <section class="sheet">
    ...
    <Teleport v-if="!session.isHr" defer to="#control-actions">
      <div class="flex items-center gap-2">
        <Button
          type="button"
          :disabled="loading || Boolean(data?.open_session)"
    ...
    <div v-else>
      ...
      <div
        v-if="!session.isHr"
        class="mb-6 grid gap-6 border-b border-border pb-6 lg:grid-cols-2 lg:gap-0"
      >
        <section
          class="lg:border-r lg:border-border lg:pr-8"
          aria-labelledby="attendance-today-title"
        >
          <div class="mb-3 flex items-center gap-2">
            <Clock3Icon class="size-5" :stroke-width="1.75" aria-hidden="true" />
            <h2 id="attendance-today-title" class="m-0">Today</h2>
          </div>
          <StatusBadge :label="todayLabel" :tone="statusTone(todayLabel)" />
          <dl v-if="currentSession" class="mt-4 grid grid-cols-[auto_1fr] gap-x-8 gap-y-2 text-sm">
            <dt class="text-muted-foreground">Check in</dt>
            <dd class="m-0 font-medium">{{ formatDateTime(currentSession.check_in_at) }}</dd>
            <dt class="text-muted-foreground">Check out</dt>
            <dd class="m-0 font-medium">{{ formatDateTime(currentSession.check_out_at) }}</dd>
          </dl>
          <p v-else class="mt-4 mb-0 text-muted-foreground">
        ...
        <section class="lg:pl-8" aria-labelledby="attendance-week-summary-title">
          <div class="mb-3 flex items-center gap-2">
            <CalendarDaysIcon class="size-5" :stroke-width="1.75" aria-hidden="true" />
            <h2 id="attendance-week-summary-title" class="m-0">This week</h2>
          </div>
          <dl class="grid gap-0 text-sm">
            <div class="flex items-center justify-between border-b border-border py-2">
              <dt class="text-muted-foreground">Recorded days</dt>
              <dd class="m-0 font-medium tabular-nums">{{ weekSummary.recorded }}</dd>
            </div>
            <div class="flex items-center justify-between border-b border-border py-2">
              <dt class="text-muted-foreground">Complete sessions</dt>
              <dd class="m-0 font-medium tabular-nums">{{ weekSummary.complete }}</dd>
            </div>
            <div class="flex items-center justify-between py-2">
              <dt class="text-muted-foreground">Needs attention</dt>
              <dd class="m-0 font-medium tabular-nums">{{ weekSummary.attention }}</dd>
            </div>
          </dl>
        </section>
        <form
          v-if="correctionOpen"
          class="mt-4 grid max-w-xl gap-3 lg:col-span-2"
      ...
      <div
        v-else-if="session.isHr"
        class="grid gap-6 lg:grid-cols-[minmax(0,1.45fr)_minmax(20rem,0.75fr)] lg:gap-0"
      >
        <div class="min-w-0 overflow-x-auto lg:border-r lg:border-border lg:pr-6">
      <Table>
      ...
      <section
        ...
        class="border-t border-border pt-5 lg:border-t-0 lg:pt-0 lg:pl-6"
        aria-labelledby="attendance-review-title"
      >
        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,0.65fr)]">
          <div>
            <p class="mb-1 text-sm text-muted-foreground">Selected exception</p>
            ...
            <dl class="grid gap-2 text-sm sm:grid-cols-2">
              <div>
                ...
                <dd class="m-0 font-medium">{{ personLabel(selectedException.employee_name) }}</dd>
              </div>
              <div>
                <dt class="text-muted-foreground">Work date</dt>
                <dd class="m-0 font-medium">{{ formatDate(selectedException.work_date) }}</dd>
              </div>
              <div>
                <dt class="text-muted-foreground">Current check in</dt>
                <dd class="m-0 font-medium">
                  {{ formatDateTime(selectedException.current_check_in_at) }}
                </dd>
              </div>
              <div>
                <dt class="text-muted-foreground">Current check out</dt>
                <dd class="m-0 font-medium">
                  {{ formatDateTime(selectedException.current_check_out_at) }}
                </dd>
              </div>
              <template v-if="selectedException.kind === 'correction_pending'">
                <div>
                  <dt class="text-muted-foreground">Proposed check in</dt>
                  <dd class="m-0 font-medium">
                    {{ formatDateTime(selectedException.proposed_check_in_at) }}
                  </dd>
                </div>
                <div>
                  <dt class="text-muted-foreground">Proposed check out</dt>
                  <dd class="m-0 font-medium">
                    {{ formatDateTime(selectedException.proposed_check_out_at) }}
                  </dd>
                </div>
              </template>
            </dl>
            <p v-if="selectedException.reason" class="mt-3 text-sm">
              <strong>Reason:</strong> {{ selectedException.reason }}
            </p>
          </div>
          <div
            v-if="selectedException.kind === 'correction_pending'"
            class="grid content-start gap-3"
          >
            <label class="grid gap-1 text-sm font-medium">
              Review comment
              <span class="font-normal text-muted-foreground">(required to reject)</span>
              <Textarea
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <Button
                ...
                >Reject correction</Button
              >
            </div>
          </div>
          <p
            v-else
            class="m-0 flex self-start gap-2 border border-[#e7c66f] bg-[#fff8dc] p-3 text-sm"
          >
