...
describe('HR people directory', () => {
  ...
  it('shows no results when the filter matches nobody', async () => {
    ...
    await inputByLabel(wrapper, 'Search people').setValue('zzz-no-match')
    await nextTick()
  ...
  it('finds an employee by work email and location', async () => {
    fetchMock.mockImplementation(() =>
      jsonResponse(200, [
        person(),
        person({
    ...
    await inputByLabel(wrapper, 'Search people').setValue('nia.shah@dayflow.demo')
    await nextTick()

    expect(wrapper.get('table').text()).toMatch(/Nia Shah/)
    expect(wrapper.get('table').text()).not.toMatch(/Rohan Iyer/)

    await inputByLabel(wrapper, 'Search people').setValue('Pune')
    await nextTick()
  ...
  it('opens a profile from the employee row', async () => {
    fetchMock.mockImplementation(() =>
      jsonResponse(200, [person({ id: OTHER_ID, first_name: 'Nia', last_name: 'Shah' })]),
    )

    const { wrapper, router } = await mountDirectory()
    await flushPromises()

    await wrapper.get('[aria-label="Open Nia Shah\'s profile"]').trigger('click')
    await flushPromises()

    expect(router.currentRoute.value.fullPath).toBe(`/employees/${OTHER_ID}`)
  })
})
...
describe('Employee profile form sheet', () => {
  ...
  it('enters edit mode, flags unsaved changes, and PATCHes permitted personal fields', async () => {
    ...
    expect(patchUrl).toContain(`/api/employees/${SELF_ID}`)
    expect(patchBody).toEqual({
      phone: '+91-90000-22222',
      address: 'Mysuru',
    })
    expect(wrapper.text()).not.toMatch(/Unsaved changes/i)
    expect((inputByLabel(wrapper, 'Phone').element as HTMLInputElement).value).toBe(
      '+91-90000-22222',
    )
  ...
  it('lets HR edit job fields and keeps salary on the Salary tab', async () => {
    ...
    expect(patchBody).toEqual({ title: 'Senior Associate' })
  ...
  it('does not show another employee payroll row on an HR profile', async () => {
    fetchMock.mockImplementation((input: RequestInfo | URL) => {
    ...
    expect(wrapper.text()).toMatch(/No payroll record/i)
    expect(wrapper.text()).not.toMatch(/99,000/)
  })
